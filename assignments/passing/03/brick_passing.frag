...
void main()
{
    ...
    // we create more 'cells'
    // making basic shape repeat itself in x and y direction
    float x = coord.x / brick_mortar_w;
    float y = coord.y / brick_mortar_h;
    //float x = coord.x;
    //float y = coord.y;

    // de determine even and odd rows
    // and then shift even rows to the right 
    ...
    // through the subtraction we get only "two steps back and forth"
    // instead of "continuous stairs"
    // I think
    x -= floor(x);
    y -= y_index;
    ...
    // the transition from the mortar to the brick
    // having a tiny bit of a gradient and not a clean cut
