...
void main()
{
    ...
    // This part define the coordinates of the pixels in each single brick, x for width, and y for height
    float x = coord.x / brick_mortar_w;
    float y = coord.y / brick_mortar_h;
    // float x = coord.x;
    // float y = coord.y;

    // this part checks if the line is even,
    // and then pushes each alternate line by 0.5 so the bricks wont be exactly in the same place
    ...
    // This part makes the brick lines repeat (I don't completly understand how, I just see the result when I change it)
    x -= floor(x);
    y -= y_index;
    ...
    // this part calculates the width and height of the mortar using smoothstep :)
  ...
  // The fun part. here we color the the bricks using vectors.
    // vec3 color = vec3(w * h);
