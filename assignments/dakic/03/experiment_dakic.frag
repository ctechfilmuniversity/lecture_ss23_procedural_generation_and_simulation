...
void main()
{
    ...
    float x = sin(coord.x / brick_mortar_w);
    float y = sin(coord.y / brick_mortar_h);
    // float x = coord.x;
    // float y = coord.y;
    ...
    vec3 color = mix(vec3(0.0, 0.0, 0.15), vec3(0.00, 0.09, 0.7), getBias(w + h, 8.2 + u_time ));
